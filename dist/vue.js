...
function defineReactive (
  obj,  // 需要reactive化的对象，比如vm.options.data
  key,  // 需要reactive化的属性
  val,     // key对应的真实变量
  customSetter
) {
  ...
  var childOb = observe(val) // 为本属性的值设置Observe
...
function initMixin (Vue) {
  ...
  Vue.prototype._init = function (options) {
    var vm = this // this复制给vm变量, this是这个vue实例自己
  ...
  function resolveConstructorOptions (vm) {
    var Ctor = vm.constructor // vm的构造函数, 一般情况下就是Vue啊, 如果是Vue.extend创建的Vue对象可能就不是了吧?
    var options = Ctor.options  // 一般情况是挂在Vue上得options默认属性, 如果是Vue.extend创建的?
    // 处理Vue.extend的场景?
    if (Ctor.super) {
